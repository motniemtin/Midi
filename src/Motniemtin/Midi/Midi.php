...
//print_r($this->midi_json);exit();
      ...
      $xml .= '    ';
        ...
        switch($msg[1]){
          ...
          case 'Off':
            ...
            $string=-1;
            $lyrics=[];
            if(isset($this->midi_json[strval($i+1)])){
              if(isset($this->midi_json[strval($i+1)][strval($t)])){
                $notes=$this->midi_json[strval($i+1)][strval($t)]["notes"];
                //print_r($notes);exit();
                foreach($notes as $note){
                  echo "$n -> ".$note['pitch']."\n";
                  if((string)$note['pitch']==(string)$n){
                    $fret=$note['fret'];
                    $string=$note['string'];
                    $lyrics=$note['lyrics'];
                  }
            ...
            foreach($lyrics as $lyric){
              $lyrics_xml.="<Lyric>$lyric</Lyric>";
            }
            $xml .= "<Note{$msg[1]} Channel=\"$ch\" Note=\"$n\" Velocity=\"$v\" Fret=\"$fret\" String=\"$string\"><Lyrics>$lyrics_xml</Lyrics></Note{$msg[1]}>\n";
            break;
  ...
  */
  ...
  //import json information for string, fret, lyrics
//   function importJson($midi_json_path){
//     $this->midi_json=json_decode(file_get_contents($midi_json_path), true);
//     // echo $midi_json_path."\n";
//     // var_dump($this->midi_json);
//     // print_r($this->midi_json);
//     // exit();
//   }
